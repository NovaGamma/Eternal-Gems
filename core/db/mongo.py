...
class DB:
    ...
    async def add_contribution(self, user, amount, source, **kwargs):
        ...
        await contributions.update_one({'user_id': user['user_id']}, {"$inc": {"amount": amount}})
    ...
    async def get_user(self, user):
        contributions = self.get_collection('contributions')
        user = await contributions.find_one({'user_id': user})
        return user

    async def sync_rsn(self, user, rsn):
        contributions = self.get_collection('contributions')
        await contributions.update_one({'user_id': user['user_id']}, {'$set': {'rsn': rsn}})
